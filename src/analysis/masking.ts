type TokenMap = {
  ...
  export const buildMasking = (args: { categories: string[]; items: string[] }) => {
    ...
    const maskText = (text: string) => {
      ...
      for (const [real, token] of cat.forward.entries()) out = replaceAllCaseInsensitive(out, real, token);
      for (const [real, token] of item.forward.entries()) out = replaceAllCaseInsensitive(out, real, token);
    ...
    const unmaskText = (text: string) => {
      ...
      for (const [token, real] of cat.reverse.entries()) out = replaceAllCaseInsensitive(out, token, real);
      for (const [token, real] of item.reverse.entries()) out = replaceAllCaseInsensitive(out, token, real);
    ...
    const unmaskPlan = <T extends Record<string, any>>(plan: T): T => {
      ...
      if (clone?.filters) {
        if ("categoryName" in clone.filters) clone.filters.categoryName = unmaskMaybe(clone.filters.categoryName);
